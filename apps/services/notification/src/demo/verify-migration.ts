/**
 * Migration-vs-synchronize schema diff for notification_db.
 *
 ...
 * documents the actual technique this repo's existing migrations were
 * verified with: two throwaway databases against the *real* running
 * `ecomiq-postgres` container — one brought up by running the hand-written
 * migration's `up()`, the other by TypeORM `synchronize:true` against the
 * same entities — then a column-level `information_schema.columns` diff
 * between them. This script clones payment's/marketing's/order's own
 * `verify-migration.ts` for notification_db, down()-loop fix included from
 * the start (no earlier single-`undoLastMigration()` bug to rediscover
 * here).
 *
 * Requires a reachable Postgres (the docker-compose `postgres` service, or
 * any Postgres matching NOTIFICATION_DB_HOST/PORT/USER/PASSWORD) — this is
 * a real-infra check, not a mocked one. Creates/drops two throwaway
 * databases (`<NOTIFICATION_DB_NAME>_verify_migrated`,
 * `<NOTIFICATION_DB_NAME>_verify_sync`); never touches the real
 * notification_db.
 *
 * Run:
 *   TS_NODE_PROJECT=apps/services/notification/tsconfig.app.json ts-node -r tsconfig-paths/register apps/services/notification/src/demo/verify-migration.ts
 */
...
const HOST = process.env.NOTIFICATION_DB_HOST ?? 'localhost';
const PORT = Number(process.env.NOTIFICATION_DB_PORT ?? 5432);
const USER = process.env.NOTIFICATION_DB_USER ?? 'ecomiq';
const PASSWORD = process.env.NOTIFICATION_DB_PASSWORD ?? 'ecomiq';
const BASE_DB = process.env.NOTIFICATION_DB_NAME ?? 'notification_db';
const MIGRATED_DB = `${BASE_DB}_verify_migrated`;
const SYNC_DB = `${BASE_DB}_verify_sync`;

const ENTITIES = [OutboxMessage, EmailTemplate, Notification, SendLog];
