/**
 * Migration-vs-synchronize schema diff for shipping_db.
 *
 ...
 * them. Clones notification-service's `verify-migration.ts`, down()-loop
 * fix included from the start.
 *
 * Requires a reachable Postgres (the docker-compose `postgres` service, or
 * any Postgres matching SHIPPING_DB_HOST/PORT/USER/PASSWORD) — this is a
 * real-infra check, not a mocked one. Creates/drops two throwaway
 * databases (`<SHIPPING_DB_NAME>_verify_migrated`,
 * `<SHIPPING_DB_NAME>_verify_sync`); never touches the real shipping_db.
 *
 * Run:
 *   TS_NODE_PROJECT=apps/services/shipping/tsconfig.app.json ts-node -r tsconfig-paths/register apps/services/shipping/src/demo/verify-migration.ts
 */
...
const HOST = process.env.SHIPPING_DB_HOST ?? 'localhost';
const PORT = Number(process.env.SHIPPING_DB_PORT ?? 5432);
const USER = process.env.SHIPPING_DB_USER ?? 'ecomiq';
const PASSWORD = process.env.SHIPPING_DB_PASSWORD ?? 'ecomiq';
const BASE_DB = process.env.SHIPPING_DB_NAME ?? 'shipping_db';
