/**
 * Migration-vs-synchronize schema diff for media_db.
 *
 ...
 * them. Clones purchasing-service's `verify-migration.ts`, down()-loop fix
 * included from the start.
 *
 * Requires a reachable Postgres (the docker-compose `postgres` service, or
 * any Postgres matching MEDIA_DB_HOST/PORT/USER/PASSWORD) — this is a
 * real-infra check, not a mocked one. Creates/drops two throwaway
 * databases (`<MEDIA_DB_NAME>_verify_migrated`, `<MEDIA_DB_NAME>_verify_sync`);
 * never touches the real media_db.
 *
 * Run:
 *   TS_NODE_PROJECT=apps/services/media/tsconfig.app.json ts-node -r tsconfig-paths/register apps/services/media/src/demo/verify-migration.ts
 */
...
const PORT = Number(process.env.MEDIA_DB_PORT ?? 5432);
const USER = process.env.MEDIA_DB_USER ?? 'ecomiq';
const PASSWORD = process.env.MEDIA_DB_PASSWORD ?? 'ecomiq';
const BASE_DB = process.env.MEDIA_DB_NAME ?? 'media_db';
const MIGRATED_DB = `${BASE_DB}_verify_migrated`;
const SYNC_DB = `${BASE_DB}_verify_sync`;

const ENTITIES = [OutboxMessage, FileFolder, ActivityLog, FileAsset];
