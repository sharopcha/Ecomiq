/**
 * Migration-vs-synchronize schema diff for purchasing_db.
 *
 ...
 * any Postgres matching PURCHASING_DB_HOST/PORT/USER/PASSWORD) — this is a
 * real-infra check, not a mocked one. Creates/drops two throwaway
 * databases (`<PURCHASING_DB_NAME>_verify_migrated`,
 * `<PURCHASING_DB_NAME>_verify_sync`); never touches the real
 * purchasing_db.
 *
 * Run:
 *   TS_NODE_PROJECT=apps/services/purchasing/tsconfig.app.json ts-node -r tsconfig-paths/register apps/services/purchasing/src/demo/verify-migration.ts
 */
...
const HOST = process.env.PURCHASING_DB_HOST ?? 'localhost';
const PORT = Number(process.env.PURCHASING_DB_PORT ?? 5432);
const USER = process.env.PURCHASING_DB_USER ?? 'ecomiq';
const PASSWORD = process.env.PURCHASING_DB_PASSWORD ?? 'ecomiq';
const BASE_DB = process.env.PURCHASING_DB_NAME ?? 'purchasing_db';
