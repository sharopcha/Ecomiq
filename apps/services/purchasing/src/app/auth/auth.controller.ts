 ...
 * one, stamped at `issue()` time). Mirrors crm's `AuthController`.
 */
@Controller('auth')
export class AuthController {
  ...
  constructor(
    ...
    this.cookieName = this.config.get<string>('PURCHASING_REFRESH_TOKEN_COOKIE_NAME', 'ecomiq_supplier_rt');
  ...
  private setRefreshCookie(res: Response, token: string) {
    res.cookie(this.cookieName, token, {
      ...
      path: '/api/purchasing/auth',
    });
  }

  private clearRefreshCookie(res: Response) {
    res.clearCookie(this.cookieName, { path: '/api/purchasing/auth' });
  }
  ...
  @Public()
  // Same brute-force/credential-guessing surface as identity's/crm's own
  // login routes — keep the global ThrottlerGuard in effect with a tighter limit.
